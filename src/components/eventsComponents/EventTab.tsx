    ...
    return (
        <ul className="lg:hidden flex flex-wrap rounded-lg px-2 py-1 text-sm bg-[#F7F7F7] font-medium text-center text-gray-500 dark:text-gray-400">
            <li
                ...
                className="mr-2">
                <button className={"inline-block py-3 px-4 rounded-lg text-[#3D195B] " + (eventTabToggle === 3 ? "bg-white font-bold": "")}>دنبال شوندگان</button>
            </li>
            <li
                onClick={() => toggleHandler(2, setEventTabToggle)}
                className="mr-2">
                <button className={"inline-block py-3 px-4 rounded-lg text-[#3D195B] " + (eventTabToggle === 2 ? "bg-white font-bold": "")}>دنبال کنندگان</button>
            </li>
            <li className="mr-2">
                <button
                    onClick={() => toggleHandler(1, setEventTabToggle)}
                    className={"inline-block py-3 px-4 rounded-lg text-[#3D195B] " + (eventTabToggle === 1 ? "bg-white font-bold": "")}>آخرین رویدادها</button>
            </li>
        </ul>
    )
}
